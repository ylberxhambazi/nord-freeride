...
class ThemifyBuilder_Data_Manager {
	...
	public static function update_builder_meta($post_id,$data,$static_text=true){
	    ...
	    if(empty($builder) || empty($builder[0]) || (empty($builder[0]['styling']) && (!isset($builder[0]['cols'][0]) || (empty($builder[0]['cols'][0]['styling']) && empty($builder[0]['cols'][0]['modules']))))){
		$builder=array();
	    }
	    elseif(isset($builder[0]) && !isset($builder[0]['element_id'])){
