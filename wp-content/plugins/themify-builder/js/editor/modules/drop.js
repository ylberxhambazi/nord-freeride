    ...
    api.Drop =  {
        row(drag, type,slug) {
            return new Promise((resolve,reject)=>{
                const rowDrop=data=>{
                    ...
                    const fragment = doc.createDocumentFragment(),
                        ...
                        callback = () => {
                            ...
                            if(type!=='pagebreak'){
                                api.correctColumnPaddings().then(()=>{
                    ...
                    for (let i = 0, len = data.length; i < len; ++i) {
                        let row = isRow===true?(new api.Row(data[i])):(new api.Subrow(data[i]));
                            fragment.appendChild(row.el);console.log(row,data);
                            rows.push(row);
                    ...
                    if (api.mode === 'visual') {
                        api.bootstrap(styles).then(callback).catch(reject);
                    } else {
                        callback();
                    }  
                };
                ...
                if (type === 'library') {
                    this.Library.row(slug).then(rowDrop).catch(reject);
                } 
                else if(type==='predesign'){
                    api.preDesignedRows.get(slug).then(rowDrop).catch(reject);
                }
                else if (type==='pagebreak') {
                    rowDrop(api.pageBreakModule.get());
                } 
                else if (type==='grid') {
                    rowDrop(api.Utils.grid(slug));
                }
                else{
                    reject();
                }
            ...
            return new Promise(resolve=>{
                ...
                */
                ...
                if (from_inner === to_inner && isDelete === false && isAdd===false) {//if it's change in the same row/subrow(only for responsive mode), just change the area order,because order is responsive
                  ...
                  //      api.Notification.showHide('tf',themifyBuilder.i18n.gridChanged.replace('%s',(Array.from(changedPoints)).join(', ')),5000);
                    }
                }
        ...
        module(drag, type,slug) {
            return new Promise((resolve,reject)=>{
                
                api.Utils.scrollTo(drag);
                //drop a new modules
                if(type === 'part' || type === 'module'){
                    this.Library.module(drag, type,slug).then(()=>{
                        resolve();
                    }).catch(reject);
                }
                else{
                    ...
                    module.trigger('edit');
                    if ('layout-part' !== slug && 'overlay-content' !== slug) {
                        if(api.mode === 'visual'){
                            module.trigger(module.getPreviewType(), _default).then(__callback);
                        }
                        else{
                            __callback();
                        }
        ...
        Library:{
            row(id){
                return new Promise((resolve,reject)=>{
                    api.Library.get(id, 'row').then(row=>{
            ...
            module(drag,type,slug){
                return new Promise((resolve,reject)=>{
                    api.Library.get(slug, type).then(options=>{
                        const module = new api.Module(options);
                        
                        api.bootstrap([module.id]).then(()=>{
                            const settings = module.get('mod_settings'),
                                __callback=()=>{
                            ...
                            module.is_new=true;
                            module.trigger('edit');
                            module.trigger(module.getPreviewType(), settings).then(__callback);
                        })
                        .catch(reject);
                    })
                    .catch(reject);
