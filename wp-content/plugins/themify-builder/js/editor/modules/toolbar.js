    ...
    api.ToolBar =  {
        ...
        init() {
            ...
            const root = doc.tfId('tb_main_toolbar_root'),
                ...
                baseCss=topWindow.document.tfId('tf_base-css').sheet.cssRules,
            ...
            cssText='';
            for(let i=0,len=baseCss.length;i<len;++i){
                cssText+=baseCss[i].cssText;
            }
            baseStyle.id='tf_base';
            baseStyle.textContent=cssText;
            doc.body.appendChild(bpSt.cloneNode(true));
            topBody.appendChild(svg.cloneNode(true));
            
            fragment.append(doc.tfId('tf_lazy_common').cloneNode(true),svg.cloneNode(true),baseStyle); 
            ...
            Themify.on('themify_builder_ready',()=>{
                    ...
                    setTimeout(() => {
                        ...
                        topWindow.Themify.loadCss(themifyBuilder.meta_url + 'css/themify.minicolors');
        ...
        setModes() {
            if (localStorage.getItem('tb_dark_mode') || win.matchMedia('(prefers-color-scheme:dark)').matches) {
        ...
        async initRevision(e) {
            ...
            const m = await Promise.all([Themify.importJs(Themify.builder_url + 'js/editor/modules/revisions'), topWindow.Themify.loadCss(Themify.builder_url + 'css/editor/modules/revisions')]);
            await m[0].Revisions.init(target);
            if (target.classList.contains('load_revision')) {
                const ul=target.closest('ul');
                    ul.style.display='none';console.log(ul);
                setTimeout(()=>{
                    ul.style.display='';
                },50);
            }
        ...
        async initLayout(e) {
            ...
            const m=await Promise.all([Themify.importJs(Themify.builder_url + 'js/editor/modules/layouts'), topWindow.Themify.loadCss(Themify.builder_url + 'css/editor/modules/layouts')]);
            await m[0].Layout.init(target);
            return m[0];
        },
        async initImport(e) {
            await api.LightBox.save();
            ...
            const m=await Themify.importJs(Themify.builder_url + 'js/editor/modules/import');
            await m.Import.init(target);
            return m;
        },
        async initExport(e){
            api.Spinner.showLoader();
            const m = await Promise.all([Themify.importJs(Themify.builder_url + 'js/editor/modules/export'),api.Helper.loadJsZip()]);
            m[0].Export.init();
            return m[0];
        },
        initHelp() {
            api.Spinner.showLoader();
            return Themify.importJs(Themify.builder_url + 'js/editor/modules/help')
            .then(m => {
                m.Help.init();
                return m;
            });
        ...
        async duplicate(e) {
            ...
            try{
                ...
                api.Notification.showHide('error',e,4000);
            }
        },
        modChange(e) {
            ...
            try{
                ...
                let lb=await api.LightBox.setStandAlone(box.left, box.top).open(options),
                    ...
                    save=async e=>{
                        ...
                        if (err && err.length > 0) {
                            await api.Notification.showHide('error',themifyBuilder.i18n.broken_code.replace('%s','CSS'),4000);
                        }
                        if (style) {
                            style.innerHTML = v;
                        }
