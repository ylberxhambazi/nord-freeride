        ...
        saveAs() {
            api.LiteLightBox.prompt(themifyBuilder.i18n.enterGlobalStyleName).then(data => {
                    if(data[0]==='yes'){
                        const title=data[1];
                        if (!title) {
                            api.Notification.showHide('error',themifyBuilder.i18n.enterGlobalStyleName);
                            this.saveAs();
                        } 
                        else {
                            ...
                            api.LocalFetch(ajaxData).then(res => {
                                ...
                                if ('success' === res.status) {
                                    ...
                                    api.Utils.saveCss(api.Helper.clear(res.data), '', res.id).then(()=>{
                                        api.Notification.showHide('done',sucessMsg);
    ...
    api.GS =  {
        ...
        init() {
            ...
            if (api.isGSPage === true) {
                ...
                if (api.mode !== 'visual' || api.is_builder_ready === true) {
                    this.openStylingPanel();
                } else {
                    topWindow.Themify.on('themify_builder_ready',() => {
                        this.openStylingPanel();
                    }, true);
                }
            } 
        ...
        openStylingPanel() {
            const _open = () => {
                ...
                const model = api.Registry.get(api.Builder.items[api.Builder.index].el.tfClass(selector)[0].dataset.cid);
				api.isPreview = false;
				model.edit('styling');
				api.isPreview = true;
                Themify.on('themify_builder_save_data', _open,true);

				const row = doc.tfId('tmpl-builder_row_action').content,
					formFields=row.querySelector('#module_form_fields_style');
				topWindow.document.body.appendChild( formFields.cloneNode(true) );
				
            };
            _open();
            api.ToolBar.el.tfClass('preview')[0].click();
        },
