(($, api, Themify, topWindow, doc) => {
    ...
    api.Drag = context => {
        ...
        .tfOn('dragstart', function (e) {
            ...
            if((isRowSort || isColumnMove || targetCl.contains('active_subrow')) && !clicked.classList.contains('tb_move')){
                e.preventDefault();
                clicked = null;
                return;
            }
                ...
                if(html || (files && files.length>0)){
                    ...
                    if(images.size>0){
                        ...
                        for(let [url,v] of images){
                            await api.Notification.show('info',uploadMsg.replaceAll('%from%',index).replaceAll('%post%',url));
                    ...
                    if(archive){
                        api.ToolBar.initImport({target:doc.createElement('div')}).then(m=>{
                           m.Import.fileImport(archive);
                        });
                    }
                }
            };
