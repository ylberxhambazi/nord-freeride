((api, doc, Themify, topWindow,und) => {
    ...
    api.MainPanel = {
        ...
        rowPanel() {
            ...
            return Promise.all([Themify.importJs(Themify.builder_url + 'js/editor/modules/predesigned-rows'), Themify.loadCss(Themify.builder_url + 'css/editor/modules/predesigned-rows', null,null, link[link.length - 3].nextElementSibling)]).then(m => {
                new api.preDesignedRows(this.el.tfClass('predesigned_container')[0]);
            });
        },
        libraryPanel() {
            const link = this.el.getRootNode().querySelectorAll('style');
            return Promise.all([Themify.importJs(Themify.builder_url + 'js/editor/modules/library'), Themify.loadCss(Themify.builder_url + 'css/editor/modules/library', null,null, link[link.length - 3].nextElementSibling)]).then(m => {
        ...
        dockMinimize() {
            ...
            this.el.style.willChange='transform,opacity,visibility';
            if(api.activeModel){
                items.push(api.LightBox.el);
                api.LightBox.el.style.willChange='opacity,visibility,'+api.isDocked;
            }
        ...
        newModule(slug) {
            ...
            api.SmallPanel.hide();
            api.Drop.module(dummy, false,slug);
        },
        newPageBreak() {
            ...
            holder.after(dummy);
            api.SmallPanel.hide();
            api.Drop.row(dummy,'pagebreak').then(()=>{
        ...
        newGrid(slug) {
            ...
            api.SmallPanel.hide();
            api.Drop.row(dummy,'grid',slug).then(()=>{
                api.undoManager.end('move');
            });
        },
        newPredesign(slug){
            ...
            holder.after(dummy);
            api.SmallPanel.hide();
            api.Drop.row(dummy,'predesign',slug).then(()=>{
                api.undoManager.end('move');
            });
        },
        ...
        initSearch() {
            ...
            if (input) {
                ...
                if (api.activeBreakPoint === 'desktop' && !Themify.isTouch) {
                    setTimeout(() => {
                        input.focus();
                    }, 250);
                }
