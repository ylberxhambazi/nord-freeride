...
(($, Themify, win, topWindow, doc,und) => {
    ...
    tb_app = {
        ...
        jsModuleLoaded(){
            if(jsModulePrms===null){
                const baseUrl=doc.currentScript.src.split('?')[0].replace('themify-builder-app','').replace('.min','').replace('.js',''),
                    notifyPrms=Themify.importJs(Themify.url+'js/admin/notification'),
                    allPromisses = [notifyPrms],
                ...
                if(api.mode==='visual'){
                    allPromisses.push(Themify.importJs('image-resize'));
                }
                notifyPrms.then(m=>{
                    api.Notification=m.Notification;
                    m.Notification.init().then(root=>{
                        if (api.mode === 'visual') {
    ...
    const api = tb_app,
	is_fullSection = !!themifyBuilder.is_fullSection;
            ...
            if (this.prms === null) {
                this.prms = new Promise((resolve, reject) => {
                    Themify.requestIdleCallback(()=>{
                        localStorage.removeItem(Clipboard.key);
                        localStorage.removeItem('themify_builder_clipboard_content');
                    },-1,5000);
                    if (this.data !== null) {
                        resolve();
                        return;
    ...
    api.Helper={
        ...
        async codeMirror(el,mode,conf){
            try{
                ...
                const {ThemifyCodeMiror}=await topWindow.Themify.importJs(Themify.url+'js/admin/modules/codemirror/codemirror'),
                obj=new ThemifyCodeMiror(el,mode,conf);
                await obj.run();
                return obj;
            }
            catch(e){
        ...
        paste(is_style,isMultiple) {
            return new Promise(async(resolve,reject)=>{
                ...
                if (data === false || (is_style && this.type==='module' && component!==data.mod_name)) {
                    api.Notification.showHide('error',themifyBuilder.i18n.text_alert_wrong_paste);
                    reject();
                    return;
                }
                if(this.type==='column' && data.modules && this.el.closest('.module_subrow')!==null){
                   for(let i=data.modules.length-1;i>-1;--i){
                       if(data.modules[i].cols!==und){
                            api.Notification.showHide('error',themifyBuilder.i18n.text_alert_sub_in_sub);
                            reject();
                            return false;
                       }
                   } 
    ...
    api.Row =class extends api.Base {;
        ...
        render() {
            ...
            if (len > 0) {
                ...
                if(len > 1){
                    ...
                    if(align===und){
                        align=is_fullSection===true?'center':'start';
                    }
                    if (align === 'start') {
                        align = 'top';
                    } else {
        ...
        getSizes(type,bp){
			...
			if(aligment===und){
				aligment=is_fullSection===true?'center':'start';
			}
        ...
        optionsTab(el) {
            ...
            ThemifyConstructor.values = prevModel && prevModel.id===this.id?api.Forms.serialize('tb_options_setting', true):(api.Helper.cloneObject(this.get('styling')) || {});
        ...
        grid(target) {
            ...
            if (bp === 'desktop') {
                ...
                if(changedPoints.length>0){
                   // api.Notification.showHide('tf',themifyBuilder.i18n.gridChanged.replace('%s',changedPoints.join(', ')),5000);
                }
            }
            else{
        ...
        setMaxGutter(bp){
            ...
            if (maxGutter < gutterVal) {
               ...
               // api.Notification.showHide('info',msg,3000);
    ...
    api.Builder=class {
        ...
        init() {
            ...
            setTimeout(() => {
                ...
                if(this.emptyModules.size>0){
                    api.Notification.showHide('warning',themifyBuilder.i18n.empty_modules.replace('%s',Array.from(this.emptyModules).join(', ')),10000);
                }
        ...
        lastRowBlock(){
            ...
            if(!this.lastRow || !this.el.contains(this.lastRow.getRootNode().host)){
                        ...
                        const fragment=doc.createDocumentFragment();
                            ...
                            this.lastRow.tfOn(Themify.click, function(e) {
                                ...
                                if (grid !== null) {
                                    ...
                                    api.MainPanel.newGrid(grid.dataset.slug);
                                } 
        ...
        reLoad(json,merge) {
            return new Promise(async resolve=>{
                ...
                if(merge===true){
                    const currentBuilder = this.toJSON(),
                        res= [];
                        for (let i in currentBuilder) {
                            res.push(currentBuilder[i]);
                        }
                        for (let i in data) {
                            res.push(data[i]);
                        }
            ...
            try{
                ...
                try{
                    ...
                    try{
                        ...
                        await Promise.all([api.Spinner.showLoader('error'),api.Notification.showHide('error',themifyBuilder.i18n.errorSaveBuilder,5000)]);
                        throw e;
                    }
                }
                res=res.data;
                res.css_file=await api.Utils.saveCss(data, customCss, id,localImages,!checkOldPadding(data));
                api.Spinner.showLoader('done');
    ...
    api.Utils = {
            ...
            if (model_r) {
                ...
                if (count>1) {
                    ...
                    for (let i = 0; i < bpLength - 1; ++i) { //clean again duplicates
                        ...
                        if (gutter || align || area || colh || size) {
                            for (let j = i + 1; j < bpLength; ++j) {
                                let bp2 = points[j];
                                ...
                                if (area && sizes[bp2 + '_area']) {
                                    if (sizes[bp2 + '_area'] === area) {
                                        delete sizes[bp + '_area'];
                                    }
                                    area = null;
                                }
        ...
        async importThemifyImages(images){
            ...
            if(images.size>0){
                return new Promise(async (resolve,reject)=>{
                    try{
                        const {ImportImages}=await Themify.importJs(Themify.url+'js/admin/import/import-images'),
                        memory=parseInt(themifyBuilder.memory) || 64,
                        chunkSize=memory>=255?4:(memory>=120?3:(memory>60?2:1)),
                        res=await ImportImages.init(images,themifyBuilder.nonce,themifyBuilder.i18n.uploading,chunkSize),
                        ...
                        api.Notification.showHide('done','',100);
                        resolve(res);
                    }
                    catch(e){
                        reject(e);
            ...
            if (api.mode === 'visual') {
                ...
                if (isAjax !== true) {
                    const d = el || doc,
                        ...
                        {ThemifyImageResize}= await Themify.importJs('image-resize');
                    for (let i = len - 1; i > -1; --i) {
        ...
        return new Promise(resolve=>{
            ...
            if(len>0){
                const colPaddingsIds=['padding_top','padding_bottom','padding_left','padding_right','margin-bottom','margin-top'],
                    ...
                    //api.Notification.show('info',themifyBuilder.i18n.convertingOldData);
                    ...
                    tmpIframe.tfOn('load', function() {
                        ...
                        this.remove();
                        tmpIframe = null;   
                  //      api.Notification.showHide('done',themifyBuilder.i18n.oldDataConverted);
                        resolve();
                    }, {
                        passive: true,
                        once: true
    ...
    win.tfOn('offline', e => {
        setTimeout(()=>{
            if(!win.navigator.onLine && !api.Builder.get().isSaved && api.undoManager.hasUndo()){
                    ...
                    if(e.composedPath()[0].closest('a')){
                        ...
                        let builder=api.Builder.get(),
                            ...
                            donwload=(blob,name)=>{
                                ...
                                a.click();
                                api.Notification.showHide('warning',msgWrap,300);
                            };
                ...
                api.Notification.show('warning',msgWrap);
                saveBtnCl.add('disabled');
                win.tfOn('online', e => {
                    saveBtnCl.remove('disabled');
                    api.Notification.showHide('warning',msgWrap,300);
                    api.Notification.el.tfOff(Themify.click,exportJson,{once:true});
                },{passive:true,once:true});
                
                api.Notification.el.tfOn(Themify.click,exportJson,{once:true});
            }
        },3000);
    },{passive:true});
