((api,Themify, $, win, doc,  topWindow,und)=> {
    ...
    const getPos=el=>{
            ...
            imageResizingModule=()=>{
              return Themify.importJs('image-resize');  
            },
            pushUndo=v=>{
                        ...
                        input.tfOn('change', async e=> {
                            ...
                            if (isImageEdit || isInline) {
                                ...
                                if ((!w && !h) || isInline) {
                                    ...
                                    const {ThemifyImageResize}=await imageResizingModule();
            ...
            imageResizing = async function (e) {
                ...
                if (activeEl && (activeEl.hasAttribute('data-w') || activeEl.hasAttribute('data-h') || activeEl.closest('[data-hasEditor]')) && (target.classList.contains('image_handler') || target.classList.contains('image_border'))) {
                            ...
                            mouseup = async function (e) {
                                ...
                                if (isMoved === true) {
                                    ...
                                    if (module) {
                                        ...
                                        if (items.length > 1) {
                                            const promises = [],
                                                {ThemifyImageResize}=await imageResizingModule();
                                            if (isLocal) {
                    ...
                    if(imageObj.isLocal===false){
                        ...
                        api.Notification.showHide('warning',themifyBuilder.i18n[msg],4000);
                    }
                    else{
                    ...
                    imageObj.lowQuality().then(()=>{
                        if (imageObj.isBig===true) {
                            api.Notification.showHide('warning',themifyBuilder.i18n.img_big.replace("%w",activeEl.naturalWidth).replace("%h",activeEl.naturalHeight),4000);
                        }
            ...
            imageMenuActions=async e=>{
                ...
                if(btn){
                    ...
                    if(action){
                        const {ThemifyImageResize}=await imageResizingModule(),
                            obj=new ThemifyImageResize(activeEl,1,1),
