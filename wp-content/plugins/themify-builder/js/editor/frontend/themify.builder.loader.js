    ...
    windowLoad = () => {
        ...
        if (!win.wp || !wp.customize) {
            let builder = doc.tfClass('themify_builder_content'),
                    toogle = doc.tfClass('toggle_tb_builder')[0],
                    found = false;
                pageId = toogle ? toogle.tfClass('tb_front_icon')[0].dataset.id: false;
                ...
                if (!toogle) {
                    toogle = doc.tfClass('js-turn-on-builder')[0];
                    if (!toogle) {
                        const edit = doc.tfClass('dashicons-edit')[0];
                        pageId = edit ? edit.dataset.id : null;
                    }
                }
                if (toogle) {
                    if (found === false) {
					...
					toogle.classList.toggle('tb_disabled_turn_on',!pageId);
                }
        }
