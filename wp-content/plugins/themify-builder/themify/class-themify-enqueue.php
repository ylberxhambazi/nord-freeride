...
class Themify_Enqueue_Assets {
	...
	echo themify_get('setting-header_html', '', true),'<script> </script>';//bug  in chrome https://bugs.chromium.org/p/chromium/issues/detail?id=332189x
    }
    ...
    public static function js_localize() {
		...
		$inline_js.=implode('',self::$wc_data);
            ...
            <!--googleoff:all-->
            <!--noindex-->
            <script id="tf_vars" defer src="data:text/javascript;base64,<?php echo base64_encode($inline_js)?>"></script>
            <!--/noindex-->
            <!--googleon:all-->
            <?php
	...
	global $wp_scripts, $wp_version;
		...
		if (is_user_logged_in()) {
            ...
            if (self::$themeVersion !== null) {
                ...
                self::$localiztion['theme_v'] = self::get_version($themeSrc, self::$themeVersion);
            }
            if (!is_admin()) {
			...
			remove_filter('embed_head', 'print_emoji_detection_script');
                    ...
                    self::$localiztion['theme_js'] = themify_enque($themeSrc);
