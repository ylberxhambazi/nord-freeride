...
((Themify,und,doc) =>{
    ...
    Skin={
        ...
        skinId:null,
        importModule(){
            return Themify.importJs(Themify.url+'js/admin/import/import');
        },
        notificationModule(){
            return Themify.importJs(Themify.url+'js/admin/notification');
        },
        getForm(data){
            ...
            return new Promise(async(resolve,reject)=>{
                const {Notification}=await this.notificationModule(),
                    ...
                    fetch=async ()=>{
                        ...
                        if(!res.success){
                            ...
                            if(err.check_license || err.buy){
                                if(err.check_license){
                                    const {License}=await Themify.importJs(Themify.url+'js/admin/modules/license');
                                    await License.init();
                                }
                        ...
                        await Notification.showHide('done',vars.labels.plugins[type+'_done'].replaceAll('%plugin%',name),1500);
                        if(type==='install'){
                    ...
                    installUpdater=async ()=>{
                        const {Install_Updater}=await Themify.importJs(Themify.url+'js/admin/modules/install-updater');
                        await Install_Updater(vars.nonce);
                    };

                await Notification.show('info',vars.labels.plugins[type].replaceAll('%plugin%',name));
                try{    
                    try{
                        await fetch();
                    }
                    ...
                    await Notification.showHide('error',vars.labels.plugins[type+'_fail'].replaceAll('%plugin%',name).replaceAll('%error%',er),4000);
                    reject(e);
                }
            });
        },
            ...
            return new Promise(async (resolve,reject)=>{
                ...
                try{
                    ...
                    if(res.menu_locations || res.theme_mods || res.product_attribute){//create menus before input menu items
                        await Notification.show('info',importMessage.menu);
                        if(res.menu){
                    ...
                    if(res.terms){
                        ...
                        res.terms=Import.sort(res.terms,'parent');
                        await Import.init(res.terms,'themify_import_terms',vars.nonce,importMessage,skinId);
                    }
                    if(res.posts){
                        importMessage.loading=importMessage.posts;
                        res.posts=Import.sort(res.posts,'post_parent');
                        await Import.init(res.posts,'themify_import_posts',vars.nonce,importMessage,skinId);
                    }
                    if(res.menu_items){
                        importMessage.loading=importMessage.menu_items;
                        res.menu_items=Import.sort(res.menu_items,'meta_input','_menu_item_menu_item_parent');
                        await Import.init(res.menu_items,'themify_import_posts',vars.nonce,importMessage,skinId);
                    }
                    ...
                    if(Object.keys(themeData.data).length>0){
                        await Notification.show('info',importMessage.theme);
                        await importThemeData(themeData);
                    }
                    await Notification.showHide('done',importMessage.done,2000);
                    resolve();
                }
                catch(e){
                    const {Notification}=await this.notificationModule(),
                        er=typeof e==='string'?e:'';
                    await Notification.showHide('error',er,2000);
                    reject(e);
                }
             });
        }
