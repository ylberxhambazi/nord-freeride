...
((Themify,win) =>{
    ...
    convert=async(find,replace,posts,total)=>{
        ...
        await Notification.show('info',getMessage(labels.searching,find,posts,count,total),1200);
    ...
    Replace=async(find,replace,nonce)=>{
        ...
        try{
            ...
            if(savingPosts.length>0){
                ...
                await Notification.show('info',getMessage(labels.found,find,savingPosts,savingPosts.length,total),3000);
                ...
                for(let i=0,len=result.length;i<len;i++){
                    ...
                    await Notification.show('info',getMessage(labels.saving,find,titles[i],summ,foundCount));
                    let r;
                    try{
                        ...
                        try{
                            ...
                            await Notification.showHide('error',e,2000);
                        }
                    }
                    if(r && r.data){
                        r=r.data;
                        ...
                        if(errors.length>0){
                            await Notification.showHide('error',getMessage(labels.no_found,find,errors,errors.length,total),4000);
                        }
                    }
                }
                if(hasSuccess===true){
                ...
                await Notification.showHide('done',labels.done);
            }
            else{
                await Notification.showHide('warning',getMessage(labels.no_found,find),3000);
            }
        }
        catch(e){
            await Notification.showHide('error',e);
        }
        win.tfOff('beforeunload',preventReload);
    };
