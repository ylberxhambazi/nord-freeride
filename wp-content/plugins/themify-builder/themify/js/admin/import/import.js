...
((Themify,win,und) =>{
    ...
    Import={
        importImages(){
          return Themify.importJs(Themify.url+'js/admin/import/import-images');  
        },
        init(posts,action,nonce,messages,id,imageAction){
            return new Promise(async (resolve,reject)=>{
                let isVisible=true;
                this.importImages();
                const {Notification}=await Themify.importJs(Themify.url+'js/admin/notification'),
                    len=posts.length,
                                ...
                                if(item.length>0){
                                    let postTitle=getTitle(item),
                                        ...
                                        if(arrLen>1 && isVisible===true){
                                            ...
                                            await Notification.show('info',msgText);
                                        }
                                        try{
                                            r=await send(item);
                                        }
                                            ...
                                            if(!r || !r.success || !r.data){
                                                if(isVisible===true){
                                                    await Notification.showHide('error',skipMsg.replaceAll('%post%',postTitle),2000);
                                                }
                                                        ...
                                                        if(item[j]!==und){
                                                            ...
                                                            if(id==itemId){
                                                                 if(respItem===false){//error, add in skip to try again at the end
                                                                    ...
                                                                    if(isVisible===true){
                                                                        await Notification.showHide('warning',skipMsg.replaceAll('%post%',getTitle([item[j]]))+':'+msg,2000);
                                                                    }
                    ...
                    try{
                        ...
                        if(images.size>0){
                            const {ImportImages}=await this.importImages();
                                allImages = await ImportImages.init([...images],nonce,messages,imageChunkSize,imageAction);
                                for(let i=posts.length-1;i>-1;--i){
                                    let builder=posts[i].meta_input?posts[i].meta_input._themify_builder_settings_json:posts[i]._themify_builder_settings_json;
                                    if(builder){
                                        if(typeof builder!=='string'){
                    ...
                    if(skipItems.size>0){//reduce chunk and trying again
                        ...
                        if(skipItems.size>0 && isVisible===true){
                            await Notification.showHide('error',messages.import_failed.replaceAll('%post%',getTitle(Array.from(skipItems))),4000);
                        }
                    }
                    if(skipItems.size===len){
                        ...
                        if(thumbs.length>0){
                            const {ImportImages}=await this.importImages();
                            try{
                                const resp=await ImportImages.init(thumbs,nonce,messages,imageChunkSize,imageAction);
