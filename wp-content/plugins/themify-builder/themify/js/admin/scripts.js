...
((Themify,$,doc)=>{
	...
	$(function() {
		...
		let link = window.location.hash && doc.querySelector( 'a[href="' + window.location.hash + '"]' );
		...
		$('.add-social-link a').on('click',e=> {
            ...
            Themify.fetch({
					...
					window.Themify_Metabox.color_picker( $(el) );
			...
			btn.tfOn( 'click', async e=>{
                const bt=e.currentTarget,
                    ...
                    if(!bt.disabled){
                        ...
                        Notification.show('info','Regenerating ...');
                        try{
                            ...
                            await Notification.showHide('done','Done',1500);
                        }
                        catch(e){
		...
		async function tb_get_builder_posts(){
            ...
            const find = doc.tfId('original_string').value.trim(),
                ...
                {Replace}= await Themify.importJs(Themify.url+'js/admin/modules/find-replace');
                return Replace(find,replace,themify_js_vars.nonce);
		}
		...
		async function themify_code_editor(el){
            try{console.log(el);
                const {ThemifyCodeMiror}=await Themify.importJs(Themify.url+'js/admin/modules/codemirror/codemirror'),
                obj=new ThemifyCodeMiror(el,'htmlmixed');
                await obj.run();
            }
            catch(e){
            }
        ...
        async function themify_erase_content(keep_modify){
            themify_show_alert();
            const {Notification}=await Themify.importJs(Themify.url+'js/admin/notification');
            await Notification.show('info',themify_js_vars.erase.processing);
            try{
                ...
                if(resp.data==='repeat'){
                    ...
                    await Notification.showHide('done',themify_js_vars.erase.done,2000);
                ...
                await Notification.showHide('error',themify_js_vars.erase.error.replace('%error%',er),4000);
                throw e; 
            }
        }
            ...
            if(target){
                ...
                try{
                    if(!cl.contains( 'disabled' )){
                        ...
                        Themify.importJs(Themify.url+'js/admin/notification');//start loading
                        
                        const moduleUrl=Themify.url+'js/admin/modules/';
                    
                        if(cl.contains('license-link') || cl.contains('update-license')){
                            const {License}=await Themify.importJs(moduleUrl+'license');
                            if(cl.contains('update-license')){
                                themify_show_alert();
                                const res=await License.update();
                            ...
                            await Install_Updater(themify_js_vars.nonce);
                            themify_hide_alert('done'); 
                            setTimeout(()=>{
                               window.location.reload();
                            },100);
                            ...
                            themify_show_alert();
                            const {Skin}=await Themify.importJs(moduleUrl+'skin'),
                            data={};
                            ...
                            const state=await Skin.init(data);
