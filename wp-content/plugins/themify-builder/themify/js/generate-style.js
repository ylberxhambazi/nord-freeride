...
(function (win, doc,und) {
    ...
    ThemifyStyles = {
        ...
        getArea: function (areaValue, cssValue, bp, count) {
            ...
            if (Object.keys(allAreas).length === 0) {
                var allCss = doc.styleSheets,
                    inner=doc.tfClass('row_inner')[0],
                    isCreated=false;
                    if(inner===und){
                        isCreated=true;
                ...
                var computed = getComputedStyle(inner),
                        regCols = /--c\d/;
                for (var i = 0, len = allCss.length; i < len; ++i) {
                    try {
                        var rules = allCss[i].cssRules;
                        if (rules) {
                            for (var j = 0, len2 = rules.length; j < len2; ++j) {
                                if (rules[j].selectorText && rules[j].selectorText.indexOf('.row_inner') !== -1 && (rules[j].cssText.indexOf('--area') !== -1 || rules[j].cssText.indexOf('--c') !== -1)) {
                                    var styles = rules[j].style;
                                    for (var k = 0, len3 = styles.length; k < len3; ++k) {
                                        if (styles[k].indexOf('--area') === 0) {
                                            allAreas[styles[k]] = computed.getPropertyValue(styles[k]).replace(/col/ig, '').replace(/\s\s+/g, ' ').trim();//we need the last rule that is why key is should be area variable
                                        } else if (styles[k].indexOf('--c') === 0 && regCols.test(styles[k])) {
                                            allSizes[styles[k]] = computed.getPropertyValue(styles[k]).replace(/\s\s+/g, ' ').replace(/\s0\./g, ' .').replace(/\.+0*?fr/g, 'fr').replace(/\.+0*?\%/g, '%').replace(/\.+0*?em/g, 'em').replace(/\.+0*?px/g, 'px').trim();
                                        }
                                    }
                                }
                            }
                        ...
                        var getCustomCss = function (component, elementId, st, allData) {
                            ...
                            if (component === 'row' || component === 'subrow') {
                                ...
                                if (allData.sizes !== und) {
                                    sizes = self.extend(true, {}, allData.sizes);
                                    ...
                                    if (gutter || align || area || colh || size) {
                                        for (var j = i + 1; j < len; ++j) {
                                            ...
                                            if (size && sizes[bp2 + '_size']) {
                                                if (sizes[bp2 + '_size'] === size && size.indexOf(' ') !== -1) {
                                                    delete sizes[bp + '_size'];
                                                }
                                                size = null;
                                            }
        ...
        fields: {
            ...
            padding: function (id, type, args, data, elementId, allData) {
                ...
                if(unit==='%' && type==='column' && split[1]===und){
                        ...
                        if (bg_values[v] !== und) {
                            if (v.indexOf('repeat') !== -1) {
                                ...
                                if (v === 'best-fit-image' || v === 'builder-parallax-scrolling' || v === 'builder-zoom-scrolling' || v === 'builder-zooming') {
                                    ...
                                    var pos = this.fields.position_box.call(this, 'background_position', type, tmp, data, elementId, allData);
                                    tmp = null;
                                    if (!pos) {
                                        pos = v === 'builder-zoom-scrolling' ? '50%' : 'background-position:center';
                                    }
                                    selector += pos + '#@#';
            ...
            grid: function (id, type, args, data, element_id, allData, bp, as_array) {
                ...
                var selectors = {},
                        ...
                        _data = allData !== null && Object.keys(allData).length > 0 ? allData : null,
					...
					var Reverse = function (area, cols) {
						...
						if (area) {
							...
							if (_data !== null &&  Themify.is_builder_active === true) {//hack to change sizes
                                var model = inner?tb_app.Registry.get(inner.closest('[data-cid]').dataset.cid):vals.model;
    ...
    if (typeof ThemifyBuilderStyle !== 'undefined') {
		...
		if (doc.readyState === 'complete') {
			...
			win.tfOn('pageshow', duplicatePage, {passive: true, once: true});
		}
