...
function themify_get_embed($url,array $args){
	...
	if($type!==false){
		...
		$allow.=';autoplay';
		$query_args['autoplay']=$args['muted']=1;
	    }
	    if(!empty($args['muted'])){
		if($type==='youtube'){
		    $query_args['mute']=1;
...
function themify_make_lazy($html, $load = true) {
	...
	if(isset($tags['img']) || isset($tags['iframe'])){//skip noscript img/iframe
		...
		if (!empty($item) && strpos($item, 'data-lazy', 4) === false &&  strpos($item, 'data-tf-src', 4) === false && strpos($item, 'data-no-script', 4) === false && strpos($item, 'data:image/', 4) === false && strpos($item, 'display:none', 4) === false && strpos($item, 'display: none', 4) === false && ($count!==0 || strpos($item, 'gravatar.com', 4) === false)) {
				...
				if ($ext === 'audio' || $ext === 'video') {
					...
					if($url && $url!==true && strpos($item,' width')===false || strpos($item,' height')===false){
						...
						if($size['w']!=='' && strpos($item,' width')===false){
						    $r[' src']='width="'.$size['w'].'" src';
						}
						if($size['h']!=='' && strpos($item,' height')===false){
						     $r[' preload']='height="'.$size['h'].'" preload';
						}
					    }
					}
					if(!empty($r)){
