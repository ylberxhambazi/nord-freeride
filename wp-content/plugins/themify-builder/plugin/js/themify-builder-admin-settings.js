((Themify,und,doc,vars) =>{
    'use strict';
    const init =async()=>{
        
        const {ThemifyPanel}=await Themify.importJs(Themify.url+'js/admin/panel'),
        el=doc.tfId('tb_panel'),
        ...
        panel.el.querySelector('#main').tfOn('submit',async e=>{
            ...
            const form = Array.from((new FormData(e.currentTarget))),
                data={},
                {Notification}=await Themify.importJs(Themify.url+'js/admin/notification');
            ...
            try{
                await  Notification.show('info','Saving');
            ...
            await Notification.showHide('done');
        });
    };
    if(window.loaded===true){
        ...
        window.tfOn('pageshow',init, {once:true, passive:true});
    }
    
})( Themify,undefined,document,tb_settings);